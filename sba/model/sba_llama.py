...
class sbaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_text_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "mm_text_tower"):
            text_tower = build_custom_model(config.mm_text_tower)
            self.text_tower = [text_tower]
        if hasattr(config, "use_mm_proj"):
            self.mm_proj = nn.Linear(text_tower.config.hidden_size, config.hidden_size)
    ...
    def initialize_text_modules(self, text_tower, mm_text_select_layer,
        ...
        if not hasattr(self, 'text_tower'):
            text_tower = build_custom_model(self.config.mm_text_tower)
        else:
    ...
    def forward(
        ...
        if text_tower is not None and (input_ids.shape[1] != 1 or self.training) and encoder_text_ids is not None:
            ...
            with torch.no_grad():
                pooled_outputs = text_tower(encoder_text_ids)
            if type(encoder_text_ids) is list:
                text_features = [self.mm_proj(pooled_outputs)[0] for pooled_outputs in pooled_outputs]
            else:
                text_features = self.mm_proj(pooled_outputs)
            # workaround for beam search at inference
            if text_features.shape[0] != input_ids.shape[0]:
                repeat = input_ids.shape[0] // text_features.shape[0]
                text_features = text_features.repeat(1, repeat).reshape(input_ids.shape[0], -1)
            if text_features.dim() == 2:
                text_features = text_features.unsqueeze(1)
            dummy_text_features = torch.zeros(text_features.shape[1], pooled_outputs.shape[-1], device=inputs_embeds.device, dtype=inputs_embeds.dtype)
                ...
                if text_tower.config.use_im_start_end:
                    ...
                    for text_start_token_pos in text_start_tokens:
                        ...
                        if orig_embeds_params is not None:
                            cur_new_input_embeds = torch.cat((cur_input_embeds[:text_start_token_pos].detach(),
                                                              cur_input_embeds[text_start_token_pos:text_start_token_pos+1],
                                                              cur_text_features, 
                                                              cur_input_embeds[text_start_token_pos + num_patches + 1:text_start_token_pos + num_patches + 2], 
                                                              cur_input_embeds[text_start_token_pos + num_patches + 2:].detach()), 
                                                              dim=0)
                        else:
...
class sbaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config):
        super(sbaLlamaForCausalLM, self).__init__(config)
